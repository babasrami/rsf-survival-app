...
with st.sidebar:
    st.markdown("### Inputs")
    # Model bundle is expected to be saved with joblib.dump(...), usually as .joblib.
    # Keep .pkl as an optional legacy extension (some users name joblib files .pkl).
    model_file = st.file_uploader(
        "Model bundle (.joblib)",
        type=["joblib", "pkl"],
        help="Upload the saved bundle (joblib.dump). Must contain keys like: model, feature_cols, scaler (optional).",
    )
...
if model_file is None:
    st.info("Upload a model bundle (.joblib) to begin.")
    st.stop()
