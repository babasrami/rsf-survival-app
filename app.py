...
def _protein_cols(cols: List[str]) -> List[str]:
    """Return columns that look like protease measurements.

    Accepts common naming variants, e.g.:
    - MMP1_pTPM / ADAM8_pTPM / ADAMTS5_pTPM
    - MMP1 / ADAM8 / ADAMTS5
    - MMP-1_pTPM / ADAM-8 (hyphen/space variants)
    """
    patt = re.compile(r"^(MMP|ADAMTS|ADAM)\s*-?\s*\d+(_pTPM)?$", re.IGNORECASE)
    out: List[str] = []
    for c in cols:
        s = str(c).strip()
        if patt.match(s):
...
def _norm_key(s: str) -> str:
    """Normalize a feature/column name for fuzzy matching."""
...
def _ensure_columns(df: pd.DataFrame, feature_cols: List[str], defaults: Optional[Dict[str, float]] = None) -> pd.DataFrame:
    ...
    # Build a relaxed lookup so we can match common naming variants.
    # Example: MMP-1 vs MMP1, with/without _pTPM suffix.
    existing = list(out.columns)
    norm_to_col: Dict[str, str] = {}
    for c in existing:
        norm_to_col[_norm_key(c)] = c

    for req in feature_cols:
        if req in out.columns:
            continue

        # 1) direct suffix swap: X <-> X_pTPM
        alias_candidates = []
        if req.endswith("_pTPM"):
            alias_candidates.append(req[: -len("_pTPM")])
        else:
            alias_candidates.append(req + "_pTPM")

        # 2) normalized lookup (handles hyphens/spaces/case)
        #    Try both the required name and its alias candidates.
        resolved = ""
        for cand in [req, *alias_candidates]:
            hit = norm_to_col.get(_norm_key(cand), "")
            if hit:
                resolved = hit
                break

        # Prefer exact-name aliases if they exist, otherwise use normalized match.
        for a in alias_candidates:
            if a and a in out.columns:
                resolved = a
                break

        if resolved and resolved in out.columns:
            out[req] = out[resolved]
        else:
            out[req] = defaults.get(req, np.nan)
...
def _apply_preprocess(
    ...
    for c in feature_cols:
        if c in fill_values and fill_values[c] is not None and not (isinstance(fill_values[c], float) and np.isnan(fill_values[c])):
            fill_map[c] = float(fill_values[c])
        else:
            med = df[c].median(skipna=True) if c in df.columns else np.nan
            fill_map[c] = float(med) if pd.notna(med) else 0.0
...
def _predict_survival(model, X: np.ndarray, years: List[float], model_time_unit: str = "days") -> Tuple[np.ndarray, np.ndarray]:
    ...
    unit = (model_time_unit or "days").strip().lower()
    # Most clinical survival datasets store duration in days. If we plot in years,
    # convert years -> model units before evaluating StepFunction.
    t_model = years_arr if unit.startswith("year") else (years_arr * 365.25)
    ...
    if hasattr(model, "predict_survival_function"):
        ...
        for i, f in enumerate(sf):
            ...
            surv[i, :] = np.clip(f(t_model), 0.0, 1.0)
    ...
    if hasattr(model, "predict_cumulative_hazard_function"):
        ...
        for i, f in enumerate(chf):
            surv[i, :] = np.exp(-np.clip(f(t_model), 0.0, np.inf))
        return years_arr, surv
...
with st.sidebar:
    ...
    model_time_unit = st.selectbox(
        "Model time unit (training)",
        options=["days", "years"],
        index=0,
        help="If the duration used to train the model is in days (common), keep 'days'.",
    )
    ...
    if bundle is None or model is None:
        ...
        try:
            if run_batch:
                ...
                t, s = _predict_survival(model, X, years, model_time_unit=model_time_unit)
